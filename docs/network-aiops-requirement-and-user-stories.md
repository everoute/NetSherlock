...
| **工具专业性** | 84 个测量工具（59 BCC + 20 bpftrace + 5 Shell），各有不同参数、过滤条件和输出格式 | 对使用者的 kernel 知识要求极高                                   |
...
我们并不缺少基础数据。Grafana 中有 5641 个 Prometheus 指标持续采集；Loki 中聚合了来自 78 个服务的日志；每个节点的 `/var/log/zbs/` 目录下保存着 pingmesh、l2ping、network-high-latency 等专项日志；OVS 有自己的日志和计数器；内核有 `/proc/net/dev`、`netstat -s` 等统计接口。数据是丰富的——但它们对 AI 模型而言，几乎是不可用的。
...
3. **日志和指标的解读方式**。`openvswitch_ovs_upcall_count` 增长快意味着 upcall 频繁，可能是 flow table miss 等原因导致包被送到 userspace 处理；`elf_vm_network_drop` 非零需要区分是 RX drop（接收端缓冲区满）还是 TX drop（发送端限速）；`node_netstat_Tcp_RetransSegs` 速率突增可能指向网络丢包但也可能是接收端应用消费慢。每个指标都有其特定的异常阈值和解读逻辑。
...
每一轮 L3 + L4 （或 L1 - L4）构成闭环的一个迭代。在 Interactive 模式下，这个迭代可以持续多轮：boundary 定界 → segment 分段 → specialized 专项分析，每一轮的输出都成为下一轮决策的输入。

#### 两层 Context 的闭环全景

一个需要明确的关键点是：**两层 Context 不是必须串联使用的——Layer 1 Context 本身就能构成独立的 ReAct 闭环，覆盖大量初步排查场景。** 只有在 Layer 1 Context 不足以定位根因时，才需要进入 Layer 2 Context 的深度诊断闭环。以下图示展示了两层 Context 如何各自独立运转，又如何协作形成完整的诊断能力：
...
│  ┌────────────────┐    ┌──────────────┐    ┌──────────────┐  │
│  │ Grafana 指标    │    │ 指标关联分析  │    │ 查询更多指标  │  │
│  │ Loki 日志      │─→  │ 告警上下文理解│ ─→ │ 关联事件/日志 │  │
│  │ Pingmesh 探测  │    │ 初步故障定界  │    │ 检查历史趋势  │  │
│  │ OVS/内核计数器 │    │              │    │ 扩大排查范围  │  │
│  └───────▲────────┘    └──────────────┘    └──────┬───────┘  │
│          └──────────── 新的 Context ◄─────────────┘          │
│                                                               │
│  ✦ 可独立运行，覆盖：告警分类、指标关联、常见问题初步定位      │
│  ✦ 许多问题可在此层闭环内解决，无需深度测量                    │
└─────────────────────────┬─────────────────────────────────────┘
                          │ 需要更精确的深层诊断数据时
                          ▼
┌──────────────── Layer 2 闭环：深度诊断 ──────────────────────┐
...
│  Context                 Intelligence          Action         │
│  ┌────────────────┐    ┌──────────────┐    ┌──────────────┐  │
│  │ 网络拓扑映射    │    │ 精确延迟归因  │    │ SSH 采集拓扑  │  │
│  │ eBPF 延迟分布  │─→  │ 丢包位置定位  │ ─→ │ 部署 eBPF 探针│  │
│  │ kfree_skb 调用栈│    │ 根因识别      │    │ 协调多点测量  │  │
│  │ vhost-TUN 时序  │    │              │    │ 切换诊断模式  │  │
│  └───────▲────────┘    └──────────────┘    └──────┬───────┘  │
│          └──────────── 新的 Context ◄─────────────┘          │
│                                                               │
│  ✦ 需主动获取，覆盖：精确测量、分段归因、根因定位、修复建议    │
│  ✦ boundary → segment → specialized 逐步深入                  │
└───────────────────────────────────────────────────────────────┘
```

**Layer 1 闭环的独立价值**。在实际运维中，大量问题可以在 Layer 1 闭环内完成初步定位甚至解决：

- **告警分类与关联**：收到延迟告警后，Agent 查询同一节点的 CPU 指标、内存使用、IO 延迟等关联指标，发现 CPU steal time 异常高——根因是宿主机超售，不需要部署 eBPF 工具就能判断
- **历史趋势对比**：Agent 查询过去 7 天的延迟趋势，发现延迟升高与某次配置变更时间吻合——根因可能是配置问题，建议回滚确认
- **多节点关联**：Agent 查询全网 pingmesh 数据，发现所有经过某个物理交换机的路径都有丢包——定界到物理网络设备，无需内核级测量

这种仅基于已有监控数据的 ReAct 闭环，在当前的实现中已经有所体现：Webhook 触发的全自动工作流中，L1 阶段查询 Grafana/Loki 数据构建初始 Context，Controller 基于 L1 Context 进行问题分类——这就是 Layer 1 闭环的确定性实现。后续通过引入 LLM 分析，Layer 1 闭环可以演进为更灵活的 ReAct 循环，在不部署任何深度工具的情况下处理更多场景。

**两层闭环的协作关系**。Layer 1 闭环给出初步定界后，如果无法确定根因（如"延迟发生在发送端内部，但不知道是 vhost 还是 OVS"），则进入 Layer 2 闭环——通过部署 eBPF 测量工具获取 Layer 1 无法触及的微秒级数据。Layer 2 闭环内部也可以迭代：boundary 定界后发现需要更细的数据，进入 segment 分段测量。两层闭环的衔接点，正是 NetSherlock 四层架构中 L1/L2 到 L3/L4 的转换。
...
| ------------------ | ---------------------------------------------------------------- | ----------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| **存储运维** | IO 延迟统计、ZBS 日志、磁盘健康度指标、存储池容量与配置          | 块级 IO 追踪（blktrace）、存储路径各阶段延迟拆解、SCSI 命令时序、缓存命中率实时采样 | 指标异常告警 → IO 模式分析 → 部署追踪工具 → 延迟归因 → 定位慢盘/热点/配置问题 → 修复建议 |
...
**已有客户数据和系统渗透是最好的起点**。监控、管理和运维工具已经部署在客户环境中运行——Grafana Dashboard 是运维人员每天查看的界面，Alertmanager 的告警通知是他们工作流的起点。我们不需要冷启动——存储指标已经在采集，集群日志已经在记录，工单系统已经在运转。每个场景的第一层 Context 已经存在，需要的是结构化组织；第二层 Context 的获取能力（深度工具集）需要建设，但可以借鉴网络场景中 eBPF 工具集的建设经验。
...
> - 多轮诊断结果通过 `checkpoint_history` 完整记录，报告中可追溯每一轮的输入和输出
...
- ✅ **Interactive 单轮（已实现）**：在 Autonomous 基础上引入 Checkpoint 机制。具体流程：

  1. L1 + L2 完成后，在 `PROBLEM_CLASSIFICATION` Checkpoint 暂停，展示分类结果供确认/修改
  2. 确认后选择测量方案，在 `MEASUREMENT_PLAN` Checkpoint 暂停，展示测量方案供确认
  3. 执行 L3 测量 + L4 分析 → 输出报告
- 🔧 **Interactive 多层递归（框架就绪）**：在单轮基础上扩展多轮 L3→L4 迭代。`CheckpointType.FURTHER_DIAGNOSIS` 已定义但尚未接入 Interactive 流程。目标流程：

  1. 单轮诊断完成后，在 `FURTHER_DIAGNOSIS` Checkpoint 暂停，展示本轮结果和下一步建议
  2. 如果专家选择继续深入，进入新一轮 L3→L4
  3. 如果专家选择终止，生成包含所有阶段历史的最终报告
- 📋 **阶段建议生成**：目标是基于 WORKFLOW_TABLE 中的升级路径（boundary → segment → specialized），结合当前诊断结果（哪个区间异常），生成结构化的下一步建议列表。当前尚未实现，规则驱动的建议生成逻辑待开发

**部分实现**

- 📋 **LLM 建议引擎**：用 LLM 驱动建议生成，实现灵活、准确的下一步推荐（需先完成多层递归基础）| 维度       | 过渡方案（规则驱动）                       | 目标状态（LLM 驱动）                                                    |
  | ---------- | ------------------------------------------ | ----------------------------------------------------------------------- |
...
| **P0** ✅ | Interactive Checkpoint 交互  | Phase 1 (done) | 单轮 L3→L4 + 2 种 Checkpoint（分类确认、测量方案确认）      |
