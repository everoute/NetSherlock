...
graph TD
    ...
    ModeCheck -->|"Autonomous"| Result["返回 DiagnosisResult"]
    ModeCheck -->|"Interactive\n(当前实现)"| Result

    subgraph FutureLoop["Phase 2 目标: Interactive 多层递归"]
        direction TB
        FutureNote["L4 完成后 →"]:::desc
        LLMAnalysis["LLM 决策引擎\n诊断结果 + KB + Skill Lib"] --> Suggest["生成建议列表"]
        Suggest --> Checkpoint{"FURTHER_DIAGNOSIS\nCheckpoint"}
        Checkpoint -->|"选定 Skill 组合"| NextPlan["更新工作流\n参数构建"]
        NextPlan --> BackToL3["→ L3"]
...
这是当前生产可用的引擎，~40 个测试文件、316+ test cases 覆盖了所有工作流路径、错误处理和边界条件。
...
Interactive 模式是连接 Phase 1 和 Phase 2 的核心能力。当前已实现单轮 Interactive 诊断（2 个 Checkpoint），多层递归是 Phase 2 的目标设计。以下描述涵盖已实现部分和目标设计。
...
**当前状态**：建议引擎尚未实现。`FURTHER_DIAGNOSIS` Checkpoint 类型已定义但未接入 Interactive 流程，建议生成逻辑待开发。规划的演进路径：先实现基于 WORKFLOW_TABLE escalation 路径的规则驱动建议（Phase 2 过渡），再演进为 LLM 综合分析后生成建议——这是后续接手的关键演进方向。

#### 场景 1: VM 延迟多层诊断（目标设计）

> **注**：以下两个 sequence 图展示的是多层递归诊断的目标设计。当前实现仅支持单轮 L3→L4（Stage 1 部分），多轮迭代（Stage 2 及 LLM 建议引擎）是 Phase 2 待实现项。
...
**可直接使用的能力**：ControllerEngine 的完整诊断链路（从告警/配置输入 → L2 拓扑采集 → L3 BPF 测量 → L4 分析归因 → 诊断报告输出）已经过实际环境验证，可以投入生产使用。Interactive 模式支持单轮 L3→L4 诊断（含问题分类和测量方案两个 Checkpoint），多层递归和建议引擎的智能化是主要演进方向。
...
- **多层递归诊断**：原计划在 Interactive 模式中实现多轮 L3→L4 迭代（boundary → segment → specialized 逐层深入），每轮结束后通过 `FURTHER_DIAGNOSIS` Checkpoint 展示建议并等待用户选择。当前 Interactive 实现为单轮 L3→L4 流程，仅在问题分类和测量方案两处设置 Checkpoint。`FURTHER_DIAGNOSIS` 已在 `CheckpointType` enum 中定义但未接入流程。偏差原因是单轮诊断已能覆盖当前 2-3 种工作流的需求，多层递归需要建议引擎支撑。这是 Phase 2 的核心演进方向。

- **LLM 建议引擎**：原计划由 LLM 综合分析诊断结果、知识库和 Skill Library 后生成下一步建议。当前尚未实现建议生成逻辑。规划的演进路径是先实现基于 WORKFLOW_TABLE escalation 路径的规则驱动建议作为过渡，再向 LLM 驱动演进。向 LLM 驱动演进是 Phase 2 的核心工作。
